...
class StrategyProviderBuilder {
   ...
   */
  inline infix fun <reified T> strategy(strategy: Strategy<T>) where T : ASTNode {
    strategy(T::class.java, strategy)
  }

  /**
   * Adds a [Strategy] for the given [ASTNode] type. In this case, the node type is passed as a parameter.
   */
  fun <T> strategy(type: Class<T>, strategy: Strategy<T>) where T : ASTNode {
